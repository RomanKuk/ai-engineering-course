...
│   ├── embed.py                 # embeddings + cache (.npy/.json)
│   ├── benchmarks/
...
# embed.py збереже окремі артефакти для документів і запитів:
# → data/doc_embeddings_<model>.npy
# → data/query_embeddings_<model>.npy
# → data/doc_ids_<model>.json
# → data/query_ids_<model>.json
